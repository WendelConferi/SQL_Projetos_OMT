...
/****** Object:  View [BiView].[ReceitaLiquida_OMT]    Script Date: 31/08/2022 07:56:02 ******/
SET ANSI_NULLS ON
GO
...
CREATE VIEW [BiView].[ReceitaLiquida_OMT]  AS
...
SELECT 
	...
	X.LINHA AS "Linha de Produto OMT",
	...
	X.FATOR * (X.VALORLINHA - X.DESCONTO + X.IPI + X.ENCARGOS + X.ICMSST)					AS "Valor Fat. Bruto",
	X.FATOR * (X.VALORLINHA - X.DESCONTO + X.IPI + ICMSST )								AS "Valor Fat. Bruto s/ Encargos",
	...
	X.FATOR * (X.ICMSST)	AS "Valor ICMSST"	
...
(
	...
	FROM FISCALDOCUMENT_BR NF
		...
		JOIN ECORESPRODUCTTRANSLATION T ON T.PRODUCT = P.RECID AND T.LANGUAGEID = 'PT-BR'
		...
		CROSS APPLY WELO.ImpostoLinha (LNF.RECID, 14) AS ICMSST --INCLUIDO 11/02 - WAGNER
		CROSS APPLY WELO.EncargoLinha (LNF.RECID) AS ENCARGO
		OUTER APPLY BiUtil.CategoriaOMT(P.RECID) AS CATEGORIA
		OUTER APPLY BiUtil.LinhaProdutoOMT(P.RECID) AS LINHAPRODUTOOMT
	...
	WHERE 1=1 
		...
		AND (( 
			...
			AND OT.OPERATIONTYPEID NOT IN (
				'S5556/6556', 'S5201/6201', 'S5410/6410', 'S5412/6412', 'S5413/6413', 'S5553/6553', 'S5556/6556', 'S5201/6201', 'S413', 'S201', 'S5206', 'DC5201', 'S5413', -- devolução de compra
	...
	WHERE 1=1 
		...
		AND (
			...
			OT.OPERATIONTYPEID IN ('F116', 'S101/102', 'S107/108', 'S118', 'S501', 'S5101', 'S5101 001', 'S5101 V.SU', 
				...
				'SFERRAM.RA', 'SFERRAMENT', 'SSUCATAS','S6107'))
	...
	FROM FISCALDOCUMENT_BR NF
		...
		CROSS APPLY WELO.ImpostoLinha (LNF.RECID, 3) AS ICOFINS
		CROSS APPLY WELO.ImpostoLinha (LNF.RECID, 4) AS IISSQN
		CROSS APPLY WELO.ImpostoLinha (LNF.RECID, 14) AS ICMSST
		CROSS APPLY WELO.EncargoLinha (LNF.RECID) AS ENCARGO
		OUTER APPLY BiUtil.CategoriaOMT(PR.RECID) AS CATEGORIA
		OUTER APPLY BiUtil.LinhaProdutoOMT(PR.RECID) AS LINHAPRODUTOOMT
		JOIN INVENTTABLEMODULE MODVENDA ON MODVENDA.ITEMID = IT.ITEMID AND MODVENDA.DATAAREAID = IT.DATAAREAID AND MODVENDA.MODULETYPE = 2 -- Venda
		JOIN INVENTTABLEMODULE MODESTOQUE ON MODESTOQUE.ITEMID = IT.ITEMID AND MODESTOQUE.DATAAREAID = IT.DATAAREAID AND MODESTOQUE.MODULETYPE = 0 -- Estoque
		JOIN LOGISTICSLOCATION LL ON LL.RECID = PT.PRIMARYADDRESSLOCATION
		JOIN LOGISTICSPOSTALADDRESS LPA ON LPA.LOCATION = LL.RECID AND CONVERT(DATE, GETDATE()) BETWEEN LPA.VALIDFROM AND LPA.VALIDTO
	...
	SELECT 
		...
		ERP.RECID AS ECORESPRODUCTID,	
		SPOT.CREATEFINANCIALTRANS,
		SPOT.CREATEINVENTTRANS,
		SCIH.CUSTTABLE,
		CFOP.CFOPID,
		CFOP.NAME AS CFOPNAME,
		IT.TAXFISCALCLASSIFICATION_BR,
		IT.ITEMID,
		ERPT.NAME AS ITEMNAME,
		ST.SALESID,
		FE.FISCALESTABLISHMENTID,	
		SCIH.DOCUMENTDATE,
		DPT.NAME AS CLIENTE,
		SCIH.FISCALDOCUMENTNUMBER,
		FDC.THIRDPARTYCNPJCPF,
		FDC.THIRDPARTYIE,
		SCIL.AMOUNT,
		0 AS DESCONTO,
		SPOT.OPERATIONTYPEID,
		SPOT.NAME AS OPERATIONNAME,

		CASE WHEN SCIH.COMPLEMENTARYTYPE = 1 THEN IIPI.VALOR					ELSE SCIL.ADDITIONALIPIAMOUNT END AS IPI,
		CASE WHEN SCIH.COMPLEMENTARYTYPE = 1 THEN IPIS.VALOR					ELSE 0 END AS PIS,
		CASE WHEN SCIH.COMPLEMENTARYTYPE = 1 THEN COALESCE(IICMS.VALOR, 0) + COALESCE(IICMSDIF.VALOR, 0) ELSE SCIL.ADDITIONALICMSAMOUNT END AS ICMS,
		CASE WHEN SCIH.COMPLEMENTARYTYPE = 1 THEN ICOFINS.VALOR					ELSE 0 END AS COFINS,
		0 AS ISSQN,
		0 AS ICMSST,
		0 AS ENCARGOS,
		1 AS FATOR,
		'Nota Fiscal Complementar de Venda' AS TIPO,
		SCIL.LINENUM AS NUMLINHA,
		CATEGORIA.CATEGORIA,
		LINHAPRODUTOOMT.LINHA,
		-- SCIL.QUANTITY, Ramon em 23-07-2020
		'0' AS QUANTITY,
		FDL.UNIT AS UNIDADENF,
		MODVENDA.UNITID AS UNIDADEVENDA,
		MODESTOQUE.UNITID AS UNIDADEESTOQUE,
		IT.NAMEALIAS,
		CT.SEGMENTID,
		CT.SUBSEGMENTID,
		CT.LINEOFBUSINESSID,
		ST.SALESGROUP AS GRUPOVENDA,
		ST.SALESPOOLID AS GRUPOOV,
		ST.PAYMENT AS CONDICAOPGTO,
		ST.PAYMENTSCHED AS PLANOPGTO,
		LPA.COUNTRYREGIONID AS PAIS,
		LPA.STATE AS ESTADO,
		LPA.CITY AS CIDADE
		
	FROM SALESCOMPLEMENTARYINVOICEHEADER SCIH 
		JOIN SALESCOMPLEMENTARYINVOICELINE SCIL ON SCIL.SALESCOMPLEMENTARYINVOICEHEADER = SCIH.RECID

		JOIN FISCALDOCUMENT_BR FDC ON FDC.RECID = SCIH.COMPLEMENTEDFISCALDOCUMENT
		JOIN CUSTINVOICEJOUR CIJ ON CIJ.RECID = FDC.REFRECID AND FDC.REFTABLEID = 62
		JOIN SALESTABLE ST ON ST.SALESID = CIJ.SALESID
		JOIN SALESTABLE_BR STBR ON STBR.SALESTABLE = ST.RECID
		JOIN SALESPURCHOPERATIONTYPE_BR SPOT ON SPOT.RECID = STBR.SALESPURCHOPERATIONTYPE_BR
		JOIN FISCALESTABLISHMENT_BR FE ON FE.RECID = SCIH.FISCALESTABLISHMENT_BR
		JOIN FISCALDOCUMENTLINE_BR FDL ON FDL.RECID = SCIL.ORIGINALFISCALDOCUMENTLINE

		JOIN CFOPTABLE_BR CFOP ON CFOP.RECID = SCIL.CFOPTABLE_BR

		JOIN CUSTTABLE CT ON CT.ACCOUNTNUM = SCIH.CUSTTABLE AND CT.DATAAREAID = SCIH.DATAAREAID
		JOIN DIRPARTYTABLE DPT ON DPT.RECID = CT.PARTY

		JOIN INVENTTABLE IT ON IT.ITEMID = SCIL.ITEMCODE AND IT.DATAAREAID = SCIL.DATAAREAID
		...
		OUTER APPLY BiWeloze.ImpostoPorGrupos (SCIL.DATAAREAID, SCIL.TAXGROUP, SCIL.TAXITEMGROUP, 8, SCIL.AMOUNT) AS IIPI
		OUTER APPLY BiWeloze.ImpostoPorGrupos (SCIL.DATAAREAID, SCIL.TAXGROUP, SCIL.TAXITEMGROUP, 1, SCIL.AMOUNT) AS IPIS
		OUTER APPLY BiWeloze.ImpostoPorGrupos (SCIL.DATAAREAID, SCIL.TAXGROUP, SCIL.TAXITEMGROUP, 2, SCIL.AMOUNT) AS IICMS
		OUTER APPLY BiWeloze.ImpostoPorGrupos (SCIL.DATAAREAID, SCIL.TAXGROUP, SCIL.TAXITEMGROUP, 15, SCIL.AMOUNT) AS IICMSDIF
		OUTER APPLY BiWeloze.ImpostoPorGrupos (SCIL.DATAAREAID, SCIL.TAXGROUP, SCIL.TAXITEMGROUP, 3, SCIL.AMOUNT) AS ICOFINS

		OUTER APPLY BiUtil.CategoriaOMT(ERP.RECID) AS CATEGORIA
		OUTER APPLY BiUtil.LinhaProdutoOMT(ERP.RECID) AS LINHAPRODUTOOMT
	...
	WHERE 1=1
		AND SCIH.DATAAREAID = 'OMT'
		AND SCIH.STATUS = 1
		AND SCIH.CANCELEDINVOICE = 0
		AND (( 
			SPOT.CREATEFINANCIALTRANS = 1 -- Movimenta financeiro
			AND SPOT.OPERATIONTYPEID NOT IN (
			...
			AND (SPOT.OPERATIONTYPEID != 'S5551/6551' OR SCIH.DATAAREAID = 'NBRA') -- desconsidera venda de ativo imobilizado, exceto para Nopin
			OR (SPOT.OPERATIONTYPEID = 'S5902/5124' AND FDL.CFOP = '5.124') -- operação especial Master
			OR (SPOT.OPERATIONTYPEID = 'T5902/5124' AND FDL.CFOP = '5.124') -- operação especial Master
			OR SPOT.OPERATIONTYPEID IN ('S116', 'S5116', 'S6116', 'S6118', 'S5116/6116', 'S5117/6117') -- inclui simples remessa
		)

) AS X

CROSS APPLY BiUtil.DataFiltroAnoMes(X.FISCALDOCUMENTDATE) 								AS DATAFILTRO
